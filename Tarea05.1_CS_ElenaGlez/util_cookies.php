...
function gestionar_cookie_familia(string $cod_familia) 
{
    if (!isset($_COOKIE["familia"])) {
        ...
        setcookie("familia[0]", $cod_familia, time() + DURATION_COOKIE_FAMILIAS);

    } else {

        // Si la cookie "familias" ya existe, comprobamos si la familia ya está presente.
        $familias_array = $_COOKIE["familia"];
        ...
        if ($index_or_found === false) {
            ...
            setcookie("familia[$count_familias]", $cod_familia, time() + DURATION_COOKIE_FAMILIAS);
        }
    }
}
